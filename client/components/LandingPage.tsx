...
function usePWAGlobal() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('pwa:installed' as any, () => { setInstalled(true); setCanInstall(false); });
  }, []);
...
function useOS() {
  ...
  useEffect(() => {
    ...
    setIsIOS(ios); setIsAndroid(android); setIsDesktop(!ios && !android);
  }, []);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass rounded-3xl p-6 border border-xena-primary/20 max-w-sm w-full text-center" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4 text-left">
          {[
            'Safari altta ortadaki Paylas (kutu+ok) tusuna basin',
            'Asagi kaydirin ve "Ana Ekrana Ekle" secenegini bulun',
            'Sag ustteki "Ekle" tusuna basin'
          ].map((t, i) => (
            <div key={i} className="flex items-start gap-3 bg-white/[0.03] rounded-2xl p-4">
              <div className="w-8 h-8 rounded-lg bg-xena-primary/15 flex items-center justify-center shrink-0 font-bold text-xena-primary">{i + 1}</div>
...
function DeskModal({ onClose }: { onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="glass rounded-3xl p-6 border border-xena-primary/20 max-w-sm w-full text-center" onClick={e => e.stopPropagation()}>
        <div className="w-16 h-16 mx-auto rounded-2xl bg-gradient-to-br from-xena-primary to-xena-accent flex items-center justify-center mb-5">
        ...
        <div className="bg-white/[0.03] rounded-2xl p-4 mb-4">
          <p className="text-xs text-xena-muted uppercase mb-2">Web Adresi</p>
          <div className="flex items-center justify-center bg-black/30 rounded-xl px-4 py-3">
            <Globe size={16} className="text-xena-primary mr-2" />
            <span className="text-lg font-bold text-white select-all">xenahub.online</span>
          </div>
        </div>
        <a href="/?app=1" className="block w-full bg-gradient-to-r from-xena-primary to-xena-accent text-white font-bold py-3 rounded-2xl active:scale-95 transition-all mb-3">PC  de Uygulama Modunu Ac</a>
...
export default function LandingPage() {
  ...
  const { canInstall, installed, prompt } = usePWAGlobal();
  ...
  return (
    <div className="min-h-dvh bg-xena-bg text-white overflow-x-hidden relative">
              ...
              <div className="w-48 h-48 mx-auto bg-white rounded-2xl p-3">
                <div className="w-full h-full bg-xena-bg rounded-xl flex items-center justify-center">
                  <div className="text-center">
                    <div className="w-16 h-16 mx-auto mb-2 border-2 border-xena-primary rounded-lg flex items-center justify-center text-2xl font-black text-xena-primary">X</div>
      ...
      {showDesk && <DeskModal onClose={() => setShowDesk(false)} />}
      ...
      <nav className="relative z-20 flex items-center justify-between px-6 md:px-12 py-6 backdrop-blur-md border-b border-white/[0.04]">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-xena-primary to-xena-accent flex items-center justify-center shadow-neon text-lg font-black">X</div>
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => setShowQR(!showQR)} className="hidden md:flex items-center gap-2 text-sm font-semibold text-xena-muted hover:text-white">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 pt-16 pb-24 grid lg:grid-cols-2 gap-16 items-center">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          ...
          <h1 className="text-5xl md:text-7xl font-black leading-[1.1] tracking-tight">
            <span className="text-white">XENAHUB</span><br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-xena-primary via-xena-accent to-xena-primary">CEO</span><br />
          ...
          <div className="flex flex-wrap items-center gap-4 mt-10">
            <button onClick={handleInstall} className="group flex items-center gap-3 bg-gradient-to-r from-xena-primary to-xena-accent text-white font-bold px-8 py-4 rounded-2xl shadow-neon active:scale-95 transition-all text-lg">
              {already ? (
            ...
            <button onClick={() => document.getElementById('features')?.scrollIntoView({ behavior: 'smooth' })} className="flex items-center gap-2 text-xena-muted hover:text-white font-semibold px-6 py-4 rounded-2xl border border-white/[0.08] hover:border-white/[0.2] active:scale-95 transition-all">
              ...
              <div className="h-full pt-10 px-5 space-y-4 overflow-hidden">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {['Yayinci','Turnuva','Sohbet'].map((l, i) => (
                    <div key={i} className="bg-white/[0.03] rounded-2xl p-3 text-center"><div className="text-lg font-black text-xena-primary">{i===0?'2K':i===1?'48':'12K'}</div><div className="text-[9px] text-xena-muted">{l}</div></div>
                  ))}
                </div>
                {[1,2,3].map(i => (
                  <div key={i} className="flex items-center gap-3 bg-white/[0.02] rounded-2xl p-3">
                    ...
                    <div className="flex-1 space-y-1"><div className="h-2.5 bg-white/10 rounded w-3/4" /><div className="h-2 bg-white/5 rounded w-1/2" /></div>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 py-10 grid grid-cols-2 md:grid-cols-4 gap-8">
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center">
              <div className="flex items-center justify-center gap-2 mb-2"><s.icon size={18} className="text-xena-primary" /><span className="text-2xl md:text-3xl font-black text-white">{s.value}</span></div>
              <p className="text-sm text-xena-muted">{s.label}</p>
            </motion.div>
          ))}
        </div>
      </div>
          ...
          {[
            ...
            <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.5 }} className="group glass rounded-3xl p-7 hover:bg-white/[0.05] transition-all duration-300 border border-white/[0.04] hover:border-xena-primary/20 cursor-default">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-xena-primary/20 to-xena-accent/10 flex items-center justify-center mb-5 group-hover:scale-110 transition-transform">
                <f.icon size={24} className="text-xena-primary" />
              </div>
              <h3 className="text-xl font-bold mb-2">{f.title}</h3>
              <p className="text-sm text-xena-muted leading-relaxed">{f.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} className="relative overflow-hidden rounded-[40px] bg-gradient-to-br from-xena-primary/20 via-xena-bg to-xena-accent/10 border border-xena-primary/20 p-10 md:p-16 text-center">
          ...
          <div className="relative">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <button onClick={handleInstall} className="group flex items-center gap-3 bg-gradient-to-r from-xena-primary to-xena-accent text-white font-bold px-10 py-5 rounded-2xl shadow-neon active:scale-95 transition-all text-lg">
                {already ? (<><CheckCircle size={24} /><span>Uygulamayi Ac</span></>) : (<><Download size={24} className="group-hover:animate-bounce" /><span>Uecretsiz Indir</span></>)}
              </button>
          ...
          <div className="grid md:grid-cols-4 gap-10 mb-12">
            <div className="md:col-span-2">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-xena-primary to-xena-accent flex items-center justify-center shadow-neon text-lg font-black">X</div>
                <div><div className="text-lg font-black">XENAHUB</div><div className="text-[10px] text-xena-muted tracking-widest uppercase">by ModClub Ajans</div></div>
              </div>
              <p className="text-sm text-xena-muted max-w-sm leading-relaxed">Turkiye  nin onde gelen yayinci platformu. Canli yayinlar, turnuvalar, etkinlikler ve topluluk.</p>
            </div>
            <div>
