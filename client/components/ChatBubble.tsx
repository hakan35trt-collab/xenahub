...
export default function ChatBubble() {
  ...
  const handleImage = (file?: File) => {
    ...
    if (file.size > 900 * 1024) return alert('Resim cok buyuk. 900KB altinda yukleyin.');
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.8, y: 10 }} className="absolute bottom-16 right-0 w-[85vw] max-w-[380px] max-h-[65dvh] glass rounded-2xl border border-white/[0.08] overflow-hidden shadow-glass">
            ...
            <div ref={scrollRef} className="px-3 py-2 space-y-2 overflow-y-auto no-scrollbar h-[37dvh]">
              ...
              {messages.map((msg) => {
                ...
                return <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}><div className={`max-w-[80%] rounded-xl px-3 py-2 text-[13px] leading-snug ${isSystem ? 'bg-xena-primary/10 text-xena-accent text-[11px]' : isMine ? 'bg-xena-primary/20 text-white' : 'bg-xena-surface text-white'}`}>{!isMine && !isSystem && <span className="text-[10px] font-bold" style={{ color: msg.senderRole && msg.senderRole in ROLE_COLORS ? ROLE_COLORS[(msg.senderRole as keyof typeof ROLE_COLORS)] : '#adadb8' }}>{msg.senderName}</span>}{msg.text && <p className="mt-0.5 whitespace-pre-wrap break-words">{msg.text}</p>}{msg.image && <img src={msg.image} className="mt-2 rounded-xl max-h-44 object-cover" />}{isMod && <button onClick={() => msg.pinned ? unpinMessage(msg.id) : pinMessage(msg.id)} className="mt-1 opacity-60 hover:opacity-100">{msg.pinned ? <PinOff size={12} className="text-xena-muted" /> : <Pin size={12} className="text-xena-muted" />}</button>}</div></div>;
              })}
            </div>
